import { ArrowLeft, ArrowRight, Calendar, Check, CheckCircle2, Copy, Home, LoaderCircle, MessageCircle, Pencil, Search } from "lucide-react";
...
const reviewLabels = {
  ...
  pet: "Pet", furnishing: "Furnishing", tenancy: "Tenancy", tenancyPeriod: "Tenancy Period", propertyUsage: "Property Usage", commercialActivity: "Commercial Activity", depositAgreement: "Deposits and Fees",
...
const validationLabels = {
  ...
  "requirements.propertyUsage": "Property Usage",
  "requirements.commercialActivity": "Commercial Activity",
...
export function RequirementPage() {
  ...
  const steps = ["Choose", "Property Basics", "Home Fit", "Preferences", ...(form.intent === "rent" ? ["Deposits & Fees"] : []), "Your Profile", "Review"];
  const finalStep = steps.length - 1;
  const profileStep = finalStep - 1;
  ...
  const next = (event) => { event?.preventDefault(); if (step === profileStep && !form.profile.race) { setErrors({ "profile.race": "Choose Malay, Chinese, Indian, or Others." }); window.setTimeout(() => document.getElementById("requirement-race-choice")?.focus(), 0); return; } if (step === 1 && !form.requirements.propertyType) { setErrors({ "requirements.propertyType": "Choose a property type." }); return; } if (step === 1 && !isValidBudgetAmount(form.requirements.budget)) { setErrors({ "requirements.budget": "Enter one positive budget amount." }); return; } if (step === 1 && !form.requirements.storeys) { setErrors({ "requirements.storeys": "Choose the number of storeys." }); return; } if (step === 2 && form.intent === "rent" && !parseRequirementDate(form.requirements.moveInDate)) { setErrors({ "requirements.moveInDate": "Enter a valid date as dd/mm/yyyy." }); window.setTimeout(() => document.getElementById("requirement-move-in-date")?.focus(), 0); return; } if (step === 3 && form.intent === "rent" && form.requirements.propertyUsage === "Commercial" && !String(form.requirements.commercialActivity || "").trim()) { setErrors({ "requirements.commercialActivity": "Please state the commercial activity." }); return; } if (step === 4 && form.intent === "rent" && !DEPOSIT_OPTIONS.includes(form.requirements.depositAgreement)) { setErrors({ "requirements.depositAgreement": "Choose Yes or Need to Discuss before reviewing your answers." }); return; } setErrors({}); if (reviewEditStep !== null) { setReviewEditStep(null); setStep(finalStep); } else setStep((current) => Math.min(finalStep, current + 1)); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  const editReviewSection = (targetStep) => {
    const stepMap = form.intent === "rent" ? { 1: profileStep, 2: 1, 3: 2, 4: 3, 5: 4 } : { 1: profileStep, 2: 1, 3: 2, 4: 3 };
    const resolvedStep = stepMap[targetStep] ?? targetStep;
    setReviewEditStep(resolvedStep);
    setErrors({});
  ...
  if (result) {
    ...
    const copyWhatsAppMessage = async () => {
      try {
        ...
        await navigator.clipboard.writeText(resultWhatsAppMessage);
        setMessageCopied(true);
        window.setTimeout(() => setMessageCopied(false), 1800);
      } catch {
        setWhatsAppError("Unable to copy the WhatsApp message. Please check browser clipboard permissions.");
      }
    };
    return (
    <main className="requirement-page"><Seo title={`Requirement ${result.reference}`} />
      <div className="page-width requirement-result">
        ...
        <section className="requirement-whatsapp requirement-next-step" aria-labelledby="requirement-whatsapp-title"><MessageCircle size={28} aria-hidden="true" /><div><span className="eyebrow">Next step</span><h2 id="requirement-whatsapp-title">Send this requirement to HS Ong</h2><p>The destination is fixed to the configured PropertyDealDesk agent number.</p></div><div className="requirement-whatsapp-actions"><a className="button primary" href={resultWhatsAppUrl || undefined} target={resultWhatsAppDesktop ? "propertydealdesk-whatsapp-business" : "_self"} rel={resultWhatsAppDesktop ? undefined : undefined} onClick={() => setWhatsAppError("")}><MessageCircle size={17} aria-hidden="true" /> WhatsApp Agent</a><button className="button secondary" type="button" onClick={copyWhatsAppMessage}>{messageCopied ? <Check size={17} aria-hidden="true" /> : <Copy size={17} aria-hidden="true" />} {messageCopied ? "Copied" : "Copy message"}</button></div></section>
    ...
    <main className="requirement-page"><Seo title="Find a Property for Me" />
      <div className="page-width requirement-shell">
        ...
        <section className="requirement-card">
          ...
          {step === profileStep ? <form onSubmit={next} className="requirement-step-panel"><StepIntro eyebrow={form.intent === "rent" ? "Tenant Profile 租客资料" : "Buyer Profile"} title={form.intent === "rent" ? "A little about you" : "A little about you"} description={form.intent === "rent" ? "May I have your basic details for owner reference? Thanks! 可以麻烦你提供以下基本信息供房主参考吗？谢谢！" : "This helps us recommend homes that fit your lifestyle."} /><div className="requirement-grid">
