...
const detailLabels = {
  propertyType: "Property Type", area: "Area / Location", budget: "Budget", bedrooms: "Bedrooms", bathrooms: "Bathrooms",
...
export function RequirementsAdminPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const { submissions = [] } = await request("/api/admin/requirements");
      setState({ loading: false, authenticated: true, submissions: Array.isArray(submissions) ? submissions : [], error: "" });
    } catch (error) {
  ...
  return (
    <main className="admin-requirements-page"><Seo title="Property Inquiries Admin" />
      <div className="page-width admin-requirements-shell">
        ...
        {state.authenticated ? <section className="admin-table-card"><div className="admin-table-wrap"><table><thead><tr><th>Reference Number</th><th>Submission Date</th><th>Rent / Buy</th><th>Name</th><th>Mobile Number</th><th>Area</th><th>Budget</th><th>Actions</th></tr></thead><tbody>{state.submissions.map((item) => <tr className={item.status === "unread" ? "unread" : ""} key={item.reference}><td><strong>{item.reference}</strong>{item.status === "unread" ? <small>New</small> : null}</td><td>{formatDateTime(item.submittedAt)}</td><td>{item.intent === "rent" ? "Rent" : "Buy"}</td><td>{item.name}</td><td>{item.mobile}</td><td>{item.area}</td><td>{formatPrice(item.budget, item.intent === "rent" ? "WTL" : "WTS")}</td><td><div className="admin-row-actions"><button type="button" title="View" onClick={() => view(item.reference)} disabled={busy === item.reference}><Eye size={16} /></button>{item.status === "unread" ? <button type="button" title="Mark as read" onClick={() => markRead(item.reference)} disabled={busy === item.reference}><MailOpen size={16} /></button> : null}<button className="danger" type="button" title="Delete" onClick={() => remove(item.reference)} disabled={busy === item.reference}><Trash2 size={16} /></button></div></td></tr>)}</tbody></table></div>{!state.submissions.length ? <div className="admin-empty">No property requirements have been submitted.</div> : null}</section> : null}
      </div>
