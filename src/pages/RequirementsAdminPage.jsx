...
import { inquiryPostingText } from "../utils/requirementPosting";
...
const detailLabels = {
  ...
  depositAgreement: "Deposits and Fees",
...
export function RequirementsAdminPage() {
  ...
  const view = async (reference) => {
    setBusy(reference);
    try {
      setSelected((await request(`/api/admin/requirements/${reference}`)).submission);
    } catch (error) {
  ...
  const markRead = async (reference) => {
    ...
    try {
      ...
      setState((current) => ({
        ...current,
        submissions: current.submissions.map((item) => item.reference === reference ? { ...item, status: "read" } : item),
      }));
  ...
  const copyPosting = async (item) => {
    ...
    try {
      let submission = item;
      try {
        submission = (await request(`/api/admin/requirements/${item.reference}`)).submission || item;
      } catch {
        submission = item;
      }
      await navigator.clipboard.writeText(inquiryPostingText(submission));
      setCopyState({ reference: item.reference, status: "copied" });
      window.setTimeout(() => setCopyState((current) => current.reference === item.reference ? { reference: "", status: "" } : current), 1800);
    } catch (error) {
      setCopyState({ reference: item.reference, status: "failed" });
      setState((current) => ({ ...current, error: "Unable to copy posting content. Please check browser clipboard permissions." }));
      window.setTimeout(() => setCopyState((current) => current.reference === item.reference ? { reference: "", status: "" } : current), 2200);
    } finally {
      setBusy("");
    }
  };
  ...
  return (
    <main className="admin-requirements-page">
      <Seo title="Property Inquiries Admin" />
              ...
              <table>
                ...
                <tbody>
                  {state.submissions.map((item) => {
                    ...
                    return (
                      <tr className={item.status === "unread" ? "unread" : ""} key={item.reference}>
                        <td><strong>{item.reference}</strong>{item.status === "unread" ? <small>New</small> : null}</td>
                        <td>{formatDateTime(item.submittedAt)}</td>
                        <td>{item.intent === "rent" ? "Rent" : "Buy"}</td>
                        <td>{item.name}</td>
                        <td>{item.mobile}</td>
                        <td>{item.area}</td>
                        <td>{formatPrice(item.budget, item.intent === "rent" ? "WTL" : "WTS")}</td>
                        <td>
                          <div className="admin-row-actions">
                            <button type="button" title="Read inquiry" aria-label={`Read inquiry ${item.reference}`} onClick={() => view(item.reference)} disabled={busy === item.reference}><Eye size={16} /></button>
                            <button
                              type="button"
                              title={copied ? "Posting copied" : copyFailed ? "Copy failed" : "Copy posting content without mobile number"}
                              aria-label={`Copy posting content for ${item.reference} without mobile number`}
                              onClick={() => copyPosting(item)}
      ...
      {selected ? (
        <div className="admin-detail-backdrop" role="presentation" onMouseDown={() => setSelected(null)}>
          <section className="admin-detail" role="dialog" aria-modal="true" aria-labelledby="submission-detail-title" onMouseDown={(event) => event.stopPropagation()}>
            <div className="admin-detail-heading">
              <div><span className="eyebrow">{selected.status === "unread" ? "Unread submission" : "Submission"}</span><h2 id="submission-detail-title">{selected.reference}</h2></div>
              <button className="icon-button" type="button" onClick={() => setSelected(null)} aria-label="Close"><X /></button>
            </div>
            <dl className="admin-detail-list">
              ...
              <div><dt>Room Summary</dt><dd>{formatRoomSummary(selected.requirements?.bedrooms, selected.requirements?.bathrooms)}</dd></div>
