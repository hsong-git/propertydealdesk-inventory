# Public property inquiries

The public inquiry form is available at `/inquiries` (the legacy `/requirements` path redirects there). It uses a strict server boundary. The browser posts to `/api/requirements`; only the Pages Function can access the dedicated `REQUIREMENTS_DB` D1 binding. The public client has no database credentials and no endpoint for reading submissions.
...
3. Create/use the D1 database named `propertydealdesk-inquiries`, then apply `migrations/0001_property_requirements.sql` to preview first, validate, then apply it to production.
