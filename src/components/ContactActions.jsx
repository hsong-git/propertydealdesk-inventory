...
export function ContactActions({ message, includeCall = true, includeEmail = false, includeWhatsApp = false, whatsappLabel = "WhatsApp", compact = false }) {
  ...
  return (
    <div className={`contact-actions ${compact ? "compact" : ""}`}>
      {includeWhatsApp ? <a className="button primary" href={whatsappUrl(agentProfile.whatsapp, whatsappMessage)} target="_blank" rel="noreferrer"><MessageCircle size={18} /> {whatsappLabel}</a> : null}
