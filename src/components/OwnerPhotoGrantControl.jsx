...
export function OwnerPhotoGrantControl({ listing = null }) {
  ...
  const idSuffix = listing?.code || "catalogue";
  return <section className="owner-photo-grant" aria-label="Owner catalogue photo download tools">
    ...
    <label htmlFor={`grant-email-${idSuffix}`}>Recipient email</label>
    <input id={`grant-email-${idSuffix}`} type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="agent@example.com" autoComplete="email" />
    ...
    {state.grant ? <div className="owner-grant-result">
      <label htmlFor={`grant-${idSuffix}`}>Valid for 24 hours, then 1 hour after first access</label>
      <div><input id={`grant-${idSuffix}`} value={state.grant.url} readOnly /><button type="button" onClick={copy} aria-label="Copy generated photo download link">{state.copied ? <Check size={18} /> : <Copy size={18} />}</button></div>
