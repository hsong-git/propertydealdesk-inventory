...
export function PhotoDownloadButton({ listing }) {
  const [state, setState] = useState({ open: false, name: "", email: "", loading: false, error: "", sent: false });
  ...
  const register = async (event) => {
    ...
    try {
      const response = await fetch(`/api/photo-download/${encodeURIComponent(listing.code)}`, { method: "POST", credentials: "include", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: state.name, email: state.email }) });
